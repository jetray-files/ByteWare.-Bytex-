...
local FPart = _G.FlingPart

DmgFlingDB = {}
function DmgFling(FTP)
	if FPart then
		pcall(function()
			for _=1,5 do
				game:GetService("RunService").RenderStepped:wait()
				FPart.CFrame = CFrame.new(FTP)
			end
			for _=1,30 do
				game:GetService("RunService").RenderStepped:wait()
				FPart.BodyPosition.Position = FTP
			end
			for _=1,5 do
				game:GetService("RunService").RenderStepped:wait()
				FPart.CFrame = CFrame.new(FTP)
...
Shotgun.Parent = game.ReplicatedStorage
	...
	for _, c in pairs(v:GetChildren()) do
		if c.ClassName == "Part" or c.ClassName == "MesPart" then
			if c.Transparency < 1 then
				local parlol = PE:Clone()
				parlol.Parent = c
				parlol.Enabled = true
				coroutine.resume(coroutine.create(function()
				parlol.Enabled = false
				Debris:AddItem(parlol,2)
				end))
			end
		end
	end
...
function meme()
	...
	if TARGET ~= nil then
		if TARGET.Parent:FindFirstChildOfClass("Humanoid") then
			...
			local mtn = Mouse.Target.Parent.Name
			if mtn ~= Character.Name and mtn ~= Character.Name and mtn ~= Player.Name and mtn ~= Player.Name and Mouse.Target.Parent:FindFirstChildOfClass("Humanoid") ~= nil then
			...
			if TORSO and HUM.Health > 0 then
					...
					coroutine.wrap(function()
						while true do
							...
							RootPart.CFrame = CFrame.new(RootPart.CFrame.p,(TORSO and Vector3.new(TORSO.CFrame.p.X,RootPart.CFrame.p.Y,TORSO.CFrame.p.Z) or ROOT and Vector3.new(ROOT.CFrame.p.X,RootPart.CFrame.p.Y,ROOT.CFrame.p.Z)))
						end
					end)()
					...
					WACKYEFFECT({Time = 18, EffectType = "Block", Size = VT(0,0,0), Size2 = VT(1,1,1), Transparency = 0, Transparency2 = 1, CFrame = CF(Hole.Position), MoveToPos = nil, RotationX = 0, RotationY = 0, RotationZ = 0, Material = "Neon", Color = C3(1,1,0), SoundID = 136523485, SoundPitch = 1, SoundVolume = 0})
					...
					WACKYEFFECT({Time = 45, EffectType = "Sphere", Size = VT(0,0,0), Size2 = VT(10,10,10), Transparency = 0, Transparency2 = 1, CFrame = TORSO.CFrame*CF(0,-1,0), MoveToPos = nil, RotationX = 0, RotationY = 0, RotationZ = 0, Material = "Neon", Color = C3(1,0,0), SoundID = 0, SoundPitch = MRANDOM(13,15)/10, SoundVolume = 0})
			...
			TORSO.CFrame = CFrame.new(MousPOS)
			TORSO.Anchored = true
			coroutine.wrap(function()
				while true do
					Swait()
					if not ATTACK then break end
					RootPart.CFrame = CFrame.new(RootPart.CFrame.p,(TORSO and Vector3.new(TORSO.CFrame.p.X,RootPart.CFrame.p.Y,TORSO.CFrame.p.Z)))
				end
			end)()
			CreateSound(429459101, TORSO, 0.5, 1, false)
			for i=0, 2, 0.1 / Animation_Speed do
